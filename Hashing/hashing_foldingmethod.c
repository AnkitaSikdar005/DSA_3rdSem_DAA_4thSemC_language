...
// Function to perform linear probing (using folding)
int linearProbing(int key, int n, int hashTable[]) {
    int loc = hashFunction(key, n);
    int i = loc;
    while (hashTable[i] != -1) {
...
// Function to perform quadratic probing (using folding)
...
// Function to perform double hashing (using folding)
int doubleHashing(int key, int n, int hashTable[], int R) {
    int loc = hashFunction(key, n);
    int i = loc;
    int h2 = R - (key % R);  // Secondary hash function (also using folding)
    while (hashTable[i] != -1) {
